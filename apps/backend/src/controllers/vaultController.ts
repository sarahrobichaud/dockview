...
export const requestContainer: RequestContainerHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  const [data, err] = await vaultReader.getProjectAssetPath(
    projectName,
    version
  );
  ...
  if (req.containerRequest.settings.provided.environment === "static-server") {
    const serverContainer = new DockviewServerContainer(projectName, version);
